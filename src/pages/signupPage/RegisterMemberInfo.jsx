...
const RegisterMajor = () => {
  ...
  return (
    <div className="flex flex-col w-full max-w-md h-full bg-white px-5 pt-10 pb-10">
      <h1 className="text-[#191F28] text-2xl font-bold tracking-tight mb-8">
        학과 등록
      </h1>

      <form onSubmit={handleMajorSubmit} className="flex flex-col gap-4 flex-1">
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            ...
            {hasError && (
              <span className="text-[#F04452] text-xs">
                학과를 선택해주세요.
              </span>
            )}
          </div>
          <div
            className={`flex items-center gap-2 ${hasError ? 'ring-2 ring-[#F04452] rounded-xl' : ''}`}
          >
            <input
              type="text"
              value={major}
        ...
        <button
          type="submit"
          disabled={!major}
          className={`w-full h-14 rounded-xl font-bold text-base border-none cursor-pointer transition-all mt-auto ${
            major
      ...
      {showModal && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-5 rounded-2xl shadow-lg z-[1001] w-[90%] max-w-[600px] h-[80%] overflow-y-auto">
            <h2 className="text-[#191F28] text-lg font-bold m-0 mb-4 tracking-tight">
              학과 선택
            </h2>
